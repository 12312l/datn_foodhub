...
const SupportPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast('error', error.response?.data?.message || 'Gửi yêu cầu thất bại');
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="card md:col-span-2">
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-gray-600 text-sm">Chọn món ăn, thêm vào giỏ hàng và tiến hành thanh toán.</p>
              </div>
              <div>
                <p className="font-medium">Làm sao để hủy đơn hàng?</p>
                <p className="text-gray-600 text-sm">Liên hệ hotline hoặc gửi yêu cầu hỗ trợ trước khi đơn được giao.</p>
              </div>
              <div>
                <p className="font-medium">Phí vận chuyển bao nhiêu?</p>
                <p className="text-gray-600 text-sm">Miễn phí vận chuyển cho đơn hàng từ 100,000 VND.</p>
              </div>
            </div>
          </div>
        </div>
